...
# def horizon_mae(preds, labels, null_val):
#     if torch.isnan(null_val):
#         mask = ~torch.isnan(labels)
#     else:
#         mask = (labels != null_val)
...
#     # loss2.requires_grad_(False)
#     # loss2=torch.mean(loss2,dim=[0,2])
#     # print(loss2.shape,loss2.requires_grad)
#     return torch.mean(loss,dim=[0,2]).detach().cpu().numpy()
...
def masked_mae_modified(preds, labels, null_val,loss_l1=0):
    if torch.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
    ...
    loss = torch.where(torch.isnan(loss), torch.zeros_like(loss), loss)
    #print(loss_l1)
    loss=torch.mean(loss) +1e-7*loss_l1
    return loss
