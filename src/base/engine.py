...
# from src.utils.metrics import horizon_mae
...
class BaseEngine():
    ...
    def train_batch(self):
        ...
        # self._dataloader['train_loader2'].shuffle()
        # self._dataloader['train_loader3'].shuffle()
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            # loss2 = horizon_mae(pred, label, mask_value)

            loss.backward()
            if self._clip_grad_value != 0:
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self._clip_grad_value)
            self._optimizer.step()
            # time_loss.append(loss2)
            train_loss.append(loss.item())
            train_mape.append(mape)
            train_rmse.append(rmse)

            self._iter_cnt += 1
        # print(np.mean(time_loss,axis=0).shape)
        return np.mean(train_loss), np.mean(train_mape), np.mean(train_rmse) #,np.mean(time_loss,axis=0)
    ...
    def train(self):
        ...
        for epoch in range(self._max_epochs):
            ...
            # mtrain_horizon=mtrain_horizon.squeeze()
            # print(mtrain_horizon.shape)
            # mtrain_array=mtrain_horizon.tostring()
            # mVal_array=mVal_horizon.tostring()
            # mVal_array=[format(x,'.4f') for x in mVal_horizon]

            # mVal_string=','.join(mVal_array)
            # mtrain_array=[format(x,'.4f') for x in mtrain_horizon]
            # mtrain_string=','.join(mtrain_array)
            # self.append_list_to_csv('val_loss.csv',mVal_horizon)
            # self.append_list_to_csv('train_loss.csv',mtrain_horizon)
    ...
    def append_list_to_csv(self,file_name, data_list):
    ...
    def evaluate(self, mode):
        ...
        if mode == 'val':
            ...
            # data = torch.zeros((12,716))
            # for i in range(12):
            #     if torch.isnan(mask_value):
            #         mask = ~torch.isnan(labels[:,i,:])
            #     else:
            #         mask = (labels[:,i,:] != mask_value)
            #     mask = mask.float()
            #     mask /= torch.mean((mask))
            #     mask = torch.where(torch.isnan(mask), torch.zeros_like(mask), mask)
            #     loss = torch.abs(preds[:,i,:]- labels[:,i,:])
            #     loss = loss * mask
            ...
            # print(data.max(),torch.mean(data))
            # print(torch.mean(data,dim=0).shape)
            # loss_mean=torch.mean(data,dim=0)
            # loss_mean=torch.reshape(loss_mean,(716,1))
            # print(loss_mean.shape)
            # self.append_list_to_csv('astgcn_loss_K13.csv',loss_mean.cpu().detach().numpy())
