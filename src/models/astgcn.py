...
class ASTGCN_block(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(temporal_At.max(),temporal_At.min(),temporal_At.shape)
        spatial_At = self.SAt(x_TAt)
        #attention=(temporal_At[0,:,:]-temporal_At[0,:,:].min())/(temporal_At[0,:,:].max()-temporal_At[0,:,:].min())
        #plt.figure(figsize=(8,6))
        #sns.heatmap(attention.detach().cpu(),cmap='crest')
        ...
        # sns.heatmap(spatial_gcn[0,0:50,0:50,0].detach().cpu(),cmap='crest')
        # plt.savefig('\home\ee\Thesis\plt.png')
        # plt.show()
...
class Spatial_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # plt.figure(figsize=(10,8))
        # sns.heatmap(S_normalized[0,0:50,0:50].detach().cpu(),cmap='crest')
        # plt.savefig('\home\ee\Thesis\plt.png')
        # plt.show()
        return S_normalized
